...
export default function UpdatePassword() {
  ...
  const onUpdatePassword = async () => {
    try {
      setLoading(true);
     await axios.post("/api/users/updatepassword", user);
  ...
  useEffect(() => {
    if (user.email === "" && user.currentPassword === "" && user.newPassword === "" ) {
...
<input
    ...
    onChange={(e) =>
        setUser((prev) => ({ ...prev, currentPassword: e.target.value }))
    }
/>
...
/>
        ...
        <Link href={"/forgetpassword1"}
        onClick={() => router.push("/forgetpassword1")}
        className="my-1 block bg-gray-300 hover:underline text-center ">Forgot Password</Link>
